...
const CustomerEditModal: React.FC<CustomerEditModalProps> = ({ isOpen, onClose, onSubmit, customer }) => {
  ...
  useEffect(() => {
    if (customer) {
      form.setFieldsValue({
        ...
        nit: customer.nit,
        day: dayjs(customer.birthDate).date(),
        month: dayjs(customer.birthDate).month() + 1, 
        year: dayjs(customer.birthDate).year(),
      });
    }
  }, [customer, form]);
  ...
  const handleOk = async () => {
    try {
      ...
      const customerData = { ...values, birthDate: formattedDate, id: customer?.id };
